...
def logger_setup():
    #在用户主目录下定义cellpose模块
    cp_dir = pathlib.Path.home().joinpath(".cellpose_")
...
def remove_model(filename, delete=False):
    """ remove model from .cellpose_ custom model list """
...
def add_model(filename):
    """ add model to .cellpose_ models folder to use with GUI or CLI """
    from . import models
...
def masks_flows_to_seg(images, masks, flows, file_names, diams=30., channels=None,
    ...
    Args:
        images (list): Images input into cellpose_.
