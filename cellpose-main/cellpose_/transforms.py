...
def average_tiles(y, ysub, xsub, Ly, Lx):
    ...
    Args:
        y (float): Output of cellpose_ network for each tile. Shape: [ntiles x nclasses x bsize x bsize]
...
def reshape(data, channels=[0, 0], chan_first=False):
    ...
    if data.shape[-1] == 1:
        ...
        if channels[0] == 0:
            ...
            data = np.concatenate((data, np.zeros_like(data)), axis=-1)
            # with open(r'D:\A001\A001.txt','w') as f:
            #     for row in data:
            #         f.write(' '.join(map(str, row)) + '\n')
        else:
            chanid = [channels[0] - 1] #[0]
            if channels[1] > 0:
                chanid.append(channels[1] - 1) #[0,0]
            data = data[..., chanid]
